...
| Mobile | Web runtime's `makeAgent` with the reception prompt; front-desk state (business, clients, appointments) and the kiosk's current date through frontend tools | Hands-free Receptionist screen (presence, STT/TTS, talking avatar, captions), `book_appointment`/`check_in_appointment` approval cards, and an owner-facing admin surface | OpenAI, Anthropic, OpenRouter or Google through the shared model resolver; STT/TTS and the vision fallback need `OPENAI_API_KEY` | Separate install; data is local to the device (AsyncStorage), no server-side store; Google session has no refresh flow; payments have no automatic verification |
